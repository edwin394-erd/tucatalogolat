...
<script>
    (function(){
        ...
        document.addEventListener('alpine:init', function(){
            ...
            if (!Alpine.store('cart')) {
                Alpine.store('cart', {
                    ...
                    save: function(){ localStorage.setItem(key, JSON.stringify(this.items)); localStorage.setItem(key + '_products', JSON.stringify(this.products)); localStorage.setItem(key + '_variants', JSON.stringify(this.variants)); localStorage.setItem(key + '_selections', JSON.stringify(this.selections)); },
                        ...
                        _syncTimer: null,
                        _syncGeneration: 0,
                        _scheduleSync: function(){ var self = this; var generation = ++self._syncGeneration; if (self._syncTimer) clearTimeout(self._syncTimer); self._syncTimer = setTimeout(function(){ try { var token = (document.querySelector('meta[name="csrf-token"]')||{}).getAttribute('content')||''; fetch('/' + routeName + '/cart-sync', { method: 'POST', headers: { 'Content-Type':'application/json', 'X-CSRF-TOKEN': token }, body: JSON.stringify({ items: self.items, products: self.products, variants: self.variants, selections: self.selections }) }).then(function(r){ return r.json(); }).then(function(data){ if (generation !== self._syncGeneration) return; var serverItems = {}; (data.items || []).forEach(function(line){ serverItems[line.line_key] = Number(line.quantity) || 0; }); Object.keys(self.items).forEach(function(lineKey){ var requested = Number(self.items[lineKey]) || 0; var accepted = serverItems[lineKey] || 0; if (requested > accepted) { window.dispatchEvent(new CustomEvent('cart-stock-limit', { detail: { productId: Number(self.products[lineKey]), requested: requested, accepted: accepted } })); } else if (self._pendingAdds[lineKey]) { window.dispatchEvent(new CustomEvent('cart-added', { detail: { message: 'Producto agregado al carrito.' } })); } if (self._pendingAdds[lineKey]) delete self._pendingAdds[lineKey]; }); if (data.items && self.hydrate) self.hydrate(data.items); window.dispatchEvent(new CustomEvent('cart-updated', { detail: { count: data.count } })); }).catch(function(){}); } catch(e){} }, 250); },
                        ...
                        decreaseLine: function(lineKey){ if (!this.items[lineKey]) return; this.items[lineKey] -= 1; if (this.items[lineKey] <= 0) { delete this.items[lineKey]; delete this.products[lineKey]; delete this.variants[lineKey]; delete this.selections[lineKey]; } this.save(); this._scheduleSync(); window.dispatchEvent(new CustomEvent('cart-updated', { detail: { count: this.count() } })); },
            ...
            window.cartDecreaseLine = function(lineKey){ if (Alpine.store('cart')) Alpine.store('cart').decreaseLine(lineKey); };

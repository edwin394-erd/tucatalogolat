...
<div class="flex flex-col lg:flex-row min-h-screen"
    ...
    {{-- Sidebar --}}
{{-- Sidebar --}}
<aside class="w-full lg:w-64 shadow-xl flex flex-col py-6 px-5 lg:fixed lg:h-full z-40 transition-colors overflow-hidden"
    ...
    {{-- Categorías --}}
    <nav class="flex-1 lg:overflow-y-auto pr-2 custom-scrollbar mt-2">
        <li class="mb-2 opacity-50 text-[10px] font-bold uppercase tracking-widest list-none" style="color: var(--text-secondary);">{{__('messages.categories')}}</li>

        <div class="lg:hidden mb-4">
            <select wire:change="filterByCategory($event.target.value)"
                    class="w-full p-2.5 rounded-xl border-none shadow-sm focus:ring-2 outline-none text-sm"
    ...
    @if (auth()->check() && auth()->user()->id === $catalogo->user_id)
        <div class="mt-auto pt-3 flex-shrink-0">
            <a wire:navigate href="{{ route('configuracion') }}"
               class="flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl text-white bg-indigo-600 text-sm font-bold shadow-lg transition-transform active:scale-95">
    ...
    {{-- Main Content --}}
    <main class="mt-4 lg:mt-0 flex-1 lg:ml-64 p-4 lg:p-6">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                ...
                <div class="relative w-full md:w-96">
                    <input type="text" wire:model.live="search"
                           class="w-full p-3 pl-11 rounded-2xl border-none shadow-sm focus:ring-2 outline-none"
                    ...
                    <svg class="absolute left-3.5 top-3.5 w-5 h-5 opacity-40" style="color: var(--text-primary);" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                </div>
            </div>

            {{-- Paginación (ahora fuera del flex del header, como bloque propio) --}}
            <div class="mb-6">
                {{ $catalogo->products->links() }}
            </div>

           {{-- Grid --}}
            <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4">
                @forelse ($catalogo->products as $item)
                    <x-product-card :catalogo="$catalogo" :item="$item" :iconColor="$iconColor" />
                @empty
                    <div class="col-span-full py-16 text-center opacity-40 font-bold" style="color: var(--text-primary);">{{__('messages.no_products')}}</div>
                @endforelse
            </div>
        </div>
    </main>

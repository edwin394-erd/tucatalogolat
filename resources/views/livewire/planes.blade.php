...
<div class="mx-auto max-w-7xl px-5 py-8 lg:px-8">
    ...
    @elseif($latestPaymentRequest && $latestPaymentRequest->payment_status === 'rejected')
        <div class="mb-6 flex items-start gap-3 rounded-2xl border border-rose-200 bg-rose-50 px-5 py-4 text-rose-950">
            ...
            <div><p class="font-bold">No pudimos validar el comprobante</p><p class="mt-1 text-sm text-rose-800">Puedes realizar el pago nuevamente y enviar un comprobante legible.</p></div>
        </div>
    @endif
                    ...
                    <div x-show="open" x-cloak x-transition.opacity @keydown.escape.window="open = false" class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm" @click.self="open = false">
                        <div class="w-full max-w-lg overflow-hidden rounded-3xl bg-white shadow-2xl" @click.stop>
                            ...
                            <div class="space-y-5 p-6">
                                ...
                                <form wire:submit.prevent="subscribe({{ $plan->id }})" class="space-y-4" @alert.window="if ($event.detail?.type === 'success') open = false">
                                    <div><label for="proof-{{ $plan->id }}" class="mb-2 block text-sm font-bold text-slate-800">Comprobante de pago</label><label for="proof-{{ $plan->id }}" class="flex cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-300 px-4 py-6 text-center transition hover:border-indigo-400 hover:bg-indigo-50"><svg class="h-8 w-8 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M12 16V4m0 0L8 8m4-4 4 4M4 16.5v1A2.5 2.5 0 0 0 6.5 20h11a2.5 2.5 0 0 0 2.5-2.5v-1"/></svg><span class="mt-2 text-sm font-semibold text-slate-700" x-text="$refs.proof.files[0]?.name || 'Seleccionar imagen o PDF'"></span><span class="mt-1 text-xs text-slate-400">JPG, PNG o PDF · máximo 5 MB</span><input x-ref="proof" id="proof-{{ $plan->id }}" type="file" wire:model="proof" accept=".jpg,.jpeg,.png,.pdf" class="sr-only"></label><x-input-error for="proof" /></div>
